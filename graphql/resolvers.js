...
const resolvers = {
  Query: {
    ...
    favouriteById: async (_, { user_id }, { dataSources }) => {
      return dataSources.favourite.getFavouriteById({ user_id }).catch((error) => catch404('ID', error))
    },
  },
  Mutation: {
    setFavouriteForUser: async(_, { user_id, character_id }, { dataSources }) => {
      return dataSources.favourite.setFavouriteById({ user_id, character_id }).catch((error) => catch404('ID', error))
    }
  },
  Character: {
